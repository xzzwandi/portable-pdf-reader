# 随身阅读

一个静态网页版 PDF/EPUB 阅读器，适合在 iPhone Chrome 或 Safari 里使用。

## 功能

- 从手机文件选择器打开本地 PDF 或 EPUB
- 分页阅读和窗口化连续阅读
- 上一页、下一页、页码跳转
- 点底部页码打开右侧全高 PDF 缩略图栏；上下拖小图逐页找，竖滑条支持全书及附近 50 页精调
- 缩略图只预览，确认后才跳页；可返回跳转前的页内位置、缩放与阅读模式，全屏也能使用
- 放大、缩小、适宽，同页缩放和全屏切换保持阅读位置
- 手机底栏常驻翻页、页码、全屏和“更多”，阅读模式与缩放位于“更多”
- 自动保存最近打开的 PDF/EPUB 和阅读进度
- 书架列表，可查看、切换、删除已保存的 PDF/EPUB
- EPUB 章节目录，可快速跳转
- PDF 内置目录、全文搜索及结果高亮、文字选择复制、按书保存页码书签
- PDF 搜索可停止，最多展示 300 处结果；扫描件需要先有 OCR 文字层
- 重复选择书架已有文件会恢复阅读进度，不再重复写入和加密
- 本地密码锁，可锁住阅读器入口
- 悬浮锁按钮，连续阅读时不用回到顶部
- PWA manifest 与基础离线缓存
- 离线更新仅在必要资源完整缓存后接管，下载失败保留旧版本
- 加密备份在覆盖原书前逐块验证正文，损坏时保留原书

连续阅读中失败的页面最多尝试 3 次（首次及 2 次自动重试），之后显示“重试本页”。书签保存在当前浏览器；未保存到书架的临时文件，书签只在本次阅读中有效。书签和阅读进度不包含在原文件导出中。

## 本地预览

```powershell
npm.cmd run start
```

电脑访问：

```text
http://127.0.0.1:5173/?v=118
```

手机访问时，手机和电脑需要在同一个 Wi-Fi 下，然后把 `127.0.0.1` 换成电脑的局域网 IP。

## 构建静态站点

```powershell
npm.cmd run build
```

构建后会生成：

```text
dist/
```

`dist` 目录就是完整静态网页，可以部署到 GitHub Pages、Cloudflare Pages、Vercel、Netlify、自己的服务器，或任何能托管静态文件的地方。

本地预览构建产物：

```powershell
npm.cmd run serve:dist
```

然后访问：

```text
http://127.0.0.1:5174/?v=118
```

## iPhone 使用

部署到 HTTPS 地址后，用手机 Chrome 或 Safari 打开网页，点「打开文件」，从“文件”App 里选择 PDF 或 EPUB。

如果想添加到主屏幕，建议用 Safari 打开 HTTPS 地址，再点分享按钮，选择“添加到主屏幕”。

## 注意

- PDF/EPUB 保存在当前浏览器对这个网址的本地存储里，不会自动上传。
- 换浏览器、换网址、清理浏览器网站数据后，已保存的 PDF/EPUB 可能不可见。
- 局域网 HTTP 地址适合测试；长期使用建议部署到 HTTPS。
- 超过 32 MiB 的解密导出和分块备份导出使用浏览器临时文件，需 HTTPS（或 localhost）及支持 OPFS 的浏览器。空间不足或不支持时会提示失败，保留书架原文件。
- 导出临时文件在下载触发约 5 分钟后清理；异常退出留下的文件会在后续大文件导出时清理（保留不到 24 小时的文件，避免打断其他页面的导出）。

## 回归验证

运行 `npm.cmd test` 验证空白页、失败重试上限、缩放和全屏阅读位置、画布内存上限、PDF 搜索与书签、重复导入续读、备份完整性、离线升级、删除失败、临时阅读进度及导出清理逻辑。

`?selftest=pdf-navigation` 使用 1200 页高矮混排 PDF，验证真实缩略图、快速更换预览候选时的进度隔离、跳转后的普通翻页与全屏切换、连续/分页精确返回，以及切书和单页文档边界。缩略图只缓存最近 24 页的低清画布，关闭时取消请求，切书或锁定时清除。

全屏回归会检查进入和退出后实际视口内已有非空画布、渲染宽度正确且阅读位置保持，无需拖动触发绘制。

`?selftest=fullscreen-scroll` 使用高矮混排的 120 页 PDF，并延迟部分页面加载；滚动到不同位置后连续切换全屏 20 次，检查可见画布、页码及页内位置，包含后台补画完成后的二次检查。切换时会暂停惯性滚动，已测页面比例跨布局保留，旧滚动校正和旧渲染队列不能影响新视图。自测模式不注册 Service Worker，便于复测最新代码。

浏览器自测使用独立的本地测试端口，避免改变日常书架的阅读状态：

```powershell
py -3 -m http.server 5183 --bind 127.0.0.1
```

访问 `http://127.0.0.1:5183/?selftest=reader-regressions`，测试空白首页、连续空白页、存储失败后的进度，以及 33 MiB 临时文件导出。页面显示「自测通过」表示完成。已有 `encrypted-switch`、`rapid-switch`、`continuous-window` 和 `fullscreen-progress` 自测仍可通过同一参数运行。
