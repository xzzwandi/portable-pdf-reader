...
- 分页阅读和窗口化连续阅读
- 上一页、下一页、页码跳转
- 点底部页码打开全屏 PDF 页面预览；选中页放大显示，上下拖小图逐页找，竖滑条支持全书及附近 50 页精调
- 缩略图只预览，确认后才跳页；可返回跳转前的页内位置、缩放与阅读模式；全屏阅读时不显示预览按钮
...
`?selftest=pdf-navigation` 使用 1200 页高矮混排 PDF，验证预览覆盖完整视口、真实缩略图、快速更换预览候选时的进度隔离、跳转后的普通翻页与全屏切换、连续/分页精确返回，以及切书和单页文档边界。页面预览只缓存最近 12 页、最大边 640px 的画布，关闭时取消请求，切书或锁定时清除。
